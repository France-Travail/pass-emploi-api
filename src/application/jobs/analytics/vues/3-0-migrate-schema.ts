...
export async function migrate(connexion: Sequelize): Promise<void> {
  ...
  await connexion.query(`
    CREATE TABLE IF NOT EXISTS ${ANALYTICS_FCT_MIGRATION_TABLE_NAME}
    (
      semaine            date,
      structure          varchar,
      type_utilisateur   varchar,
...
`)
  ...
  await connexion.query(`
    ...
    create index if not exists ${ANALYTICS_FCT_MIGRATION_TABLE_NAME}_semaine_index on ${ANALYTICS_FCT_MIGRATION_TABLE_NAME} (semaine);
    create index if not exists ${ANALYTICS_FCT_MIGRATION_TABLE_NAME}_structure_index on ${ANALYTICS_FCT_MIGRATION_TABLE_NAME} (structure);
    create index if not exists ${ANALYTICS_FCT_MIGRATION_TABLE_NAME}_type_utilisateur_index on ${ANALYTICS_FCT_MIGRATION_TABLE_NAME} (type_utilisateur);
    create index if not exists ${ANALYTICS_FCT_MIGRATION_TABLE_NAME}_categorie_index on ${ANALYTICS_FCT_MIGRATION_TABLE_NAME} (categorie);
    create index if not exists ${ANALYTICS_FCT_MIGRATION_TABLE_NAME}_action_index on ${ANALYTICS_FCT_MIGRATION_TABLE_NAME} (action);
    create index if not exists ${ANALYTICS_FCT_MIGRATION_TABLE_NAME}_nom_index on ${ANALYTICS_FCT_MIGRATION_TABLE_NAME} (nom);
