...
function verifierProfilBeneficiaire(
  ...
  // FT Connect : bouton unique côté bénéficiaire, le dispositif se lit en base
  if (estFranceTravail(command.profil.structure)) {
  ...
  if (!memeProfil(command.profil, utilisateurTrouve.profil)) {
    return failure(
      new NonTraitableError(
        'Utilisateur',
        command.idUtilisateurAuth,
