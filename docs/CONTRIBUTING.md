...
- pass-emploi-connect : Serveur d'authentification

### Services externes <a name="services-externes"></a>

- Brevo : Templates & Envois d'emails
- francetravail.io
- api milo
- firebase
...
1. Créer une entrée jeune en base de données (pass-emploi-api-xxx)
...
- Controllers
  - On utilise l'utilitaire **_TestingModuleBuilder_** de NestJS pour monter l'application afin de tester les routes.
...
_Les tests sans base de données sont lancés en parallèle pour aller plus vite, ceux ayant une adherence avec la base de données en série_
...
yarn watch:staging    # DB staging (tunnel Scalingo à lancer à côté)
```
...
variables nommées du vault (`DATABASE_URL_LOCAL`, `DATABASE_URL_STAGING`, `OIDC_ISSUER_URL_LOCAL`).
...
2. Ajouter sa clé ssh sur scalingo
   1. User settings > pass-emploi-api-prod droits
...
L'environnement de staging front correspond à l'application scalingo front `pass-emploi-api-staging`.
...
Il est également possible de déployer manuellement en allant sur `pass-emploi-api-staging > Deploy > Manual deployments > Trigger deployment`

Les review apps sont activés sur cet environnement. Donc, à chaque nouvelle PR sur develop, une application temporaire au nom `pass-emploi-api-staging-pr[numéro de la PR sur github]` sera automatiquement créée. Cette application sera automatiquement détruite au merge de la PR.
...
L'environnement de prod front correspond à l'application scalingo back `pass-emploi-api-prod`.

Cette application est branchée sur la branche `master` du repo.
Le déploiement automatique est désactivé : il faut alors aller sur scalingo et suivre les étapes suivantes `pass-emploi-api-prod > Deploy > Manual deployments > Trigger deployment`
